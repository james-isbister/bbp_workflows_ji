...
import spikewriter
from pyramid_patterns import setup_base_patterns
...
### [MODIFIED from A. Ecker's code base] ###
def _setup_pyramid_base_patterns(path, c, proj_name, mask, supersample, max_dist, n_clusters, pct, seed):
    """Local helper to avoid looping `pyramid_patterns.setup_pyramid_patterns()` by saving the results
    and next time loading the saved results instead of recalculating the whole thing again"""
    supersample_str = "__supersample" if supersample else ""
    vpm_pct_str = ("pct%.2f" % pct).replace('.', 'p') if isinstance(pct, float) else "pct%i" % pct
    save_name = os.path.join(os.path.split(path)[0], "input_spikes", "%s__%s%s__nc%i__%s__seed%i__pattern_nids.pkl" %
                             (proj_name, mask, supersample_str, n_clusters, vpm_pct_str, seed))
    if not os.path.isfile(save_name):
        # Get spatial location of fibers and cluster them to "bundles"
        nids, pos = _get_projection_locations(path, c, proj_name, mask, supersample)
        mask_nids, mask_pos = projloc.mask_location_by_dist(nids, pos, max_dist=max_dist)
        _, _, grp_idx = projloc.cluster_by_locations(mask_nids, mask_pos, n_clusters=n_clusters, cluster_seed=seed)
        # assign (groups of) nids to patterns
        pattern_nids = setup_base_patterns(mask_nids, deepcopy(grp_idx), pct, save_name, seed)
    else:
        with open(save_name, "rb") as f:
            pattern_nids = pickle.load(f)
    return pattern_nids
...
### [MODIFIED from A. Ecker's code base] ###
def generate_pyramid_base_stim(*, path, **kwargs):
    """Generates VPM and (optionally) POm spike trains, in which VPM fiber bundles form 8 distinct patterns
    (8 base patterns w/o overlap)"""
    param_list = ["circuit_config", "circuit_target", "user_target_path",  # base circuit
                  "stim_seed", "vpm_pct", "vpm_proj_name", "supersample", "max_dist", "n_clusters",  # VPM fibers
                  "pom_pct", "pom_proj_name",  # POm fibers
                  "sim_duration", "reconn_delay", "stim_start", "stim_end", "stim_rate",  # stim. series
                  "stim_duration", "min_rate", "vpm_max_rate",  # VPM rate (POm rate is derived from these)
                  "bq", "tau"]  # spikes
    cfg = {p: kwargs.get(p) for p in param_list}
    stim_file_dict = {}
    rate_start = cfg["reconn_delay"] if cfg["reconn_delay"] != cfg["sim_duration"] else cfg["stim_start"] - 1000.
    # Load circuit and fix targets
    c = Circuit(cfg["circuit_config"])
    # VPM spike train
    if cfg["vpm_proj_name"]:
        assert cfg["vpm_pct"] > 0.0, "ERROR: No VPM fibers selected!"
        vpm_node_set = _find_popul_node_set(c, cfg["vpm_proj_name"])  # Assuming there exists exactly one node set corresponding to given projection (=virtual population)
        pattern_nids = _setup_pyramid_base_patterns(path, c, cfg["vpm_proj_name"], cfg["circuit_target"], cfg["supersample"],
                                               cfg["max_dist"], cfg["n_clusters"], cfg["vpm_pct"], cfg["stim_seed"])
        stim_times = _generate_rnd_pattern_stim_series(path, cfg["stim_start"], cfg["stim_end"]+1, cfg["stim_rate"],
                                                       list(pattern_nids.keys()), cfg["stim_seed"])
        t, stim_rate = spikewriter.generate_rate_signal(rate_start, cfg["sim_duration"], stim_times,
                                                        cfg["stim_duration"], cfg["min_rate"], cfg["vpm_max_rate"])
        vpm_spike_times, vpm_spiking_nids = spikewriter.generate_inh_adaptingmarkov_spike_train(pattern_nids, t, stim_rate,
        ...
        vpm_spiking_gids = np.array(vpm_spiking_nids) + 1  # IMPORTANT: Convert SONATA node IDs (0-based) to NEURON cell IDs (1-based)!!
                                                           # (See https://sonata-extension.readthedocs.io/en/latest/blueconfig-projection-example.html#dat-spike-files)
        spikewriter.write_spikes(vpm_spike_times, vpm_spiking_gids, os.path.join(path, vpm_stim_file))
        stim_file_dict.update({"vpm_stim_file": vpm_stim_file, "vpm_node_set": vpm_node_set})

    # POm spike train
    if cfg["pom_proj_name"]:
        assert cfg["pom_pct"] > 0.0, "ERROR: No POm fibers selected!"
        pom_node_set = _find_popul_node_set(c, cfg["pom_proj_name"])  # Assuming there exists exactly one node set corresponding to given projection (=virtual population)
        pom_max_rate = 0.5 * cfg["vpm_max_rate"]
        pom_nids, _ = _get_projection_locations(path, c, cfg["pom_proj_name"], cfg["circuit_target"], False)
        np.random.seed(cfg["stim_seed"])
        pom_nids = np.sort(np.random.choice(pom_nids, size=int(len(pom_nids) * cfg["pom_pct"]/100.), replace=False))
        pom_stim_times = spikewriter.extract_stim_times(stim_times)
        _, pom_rate = spikewriter.generate_rate_signal(rate_start, cfg["sim_duration"], pom_stim_times,
                                                       cfg["stim_duration"], cfg["min_rate"], pom_max_rate)
        pom_spike_times, pom_spiking_nids = spikewriter.generate_inh_adaptingmarkov_spike_train(pom_nids, t, pom_rate,
                                                        cfg["bq"], cfg["tau"], cfg["stim_seed"])
        # Write to file and return file name used in the template
        pom_stim_file = "pom_input.dat"
        pom_spiking_gids = np.array(pom_spiking_nids) + 1  # IMPORTANT: Convert SONATA node IDs (0-based) to NEURON cell IDs (1-based)!!
