...
def sample_presyn_targets_L23(circuit, target, morpho_class, sample=10):
    ...
    cell_group_2 = circuit.cells.ids({'$target': target, Cell.LAYER: 2, Cell.MORPH_CLASS: morpho_class}, sample=init_sample)
    cell_group_3 = circuit.cells.ids({'$target': target, Cell.LAYER: 3, Cell.MORPH_CLASS: morpho_class}, sample=init_sample)
    ...
    while len(previously) < N:
        ...
        if sample_next.index[0] != sample_current.index[0]:      
            ...
            if check_distance(sample_next.iloc[0], xyzs.loc[previously]):
                ...
                xyzs.drop(sample_current.index[0])    
    

    sample_all = previously
    return sample_all
